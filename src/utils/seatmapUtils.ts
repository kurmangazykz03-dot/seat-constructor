 ...
 */
export function moveSelectedElements(state: SeatmapState, selectedIds: string[], dx: number, dy: number): SeatmapState {
  // Определяем, какие зоны, ряды и места нужно переместить
  ...
  // Добавляем дочерние элементы выделенных объектов
  ...
  return {
    ...
    rows: state.rows.map(r =>
      rowIdsToMove.has(r.id) ? { ...r, x: r.x + dx, y: r.y + dy } : r
    ),
